"""
PocketERPIDE Web MVP screenshot generator.
Headless chromium ile /pocket route'undaki 4 sekmeyi 1920x1080 olarak yakalar.
Auth bypass: /api/shop/auth/me intercept + localStorage'a demo veri inject.

Output: public/screenshots/pocketerpide/{01,02,03,04}-<tab>.png
"""
import asyncio
import json
import os
...
# Console UTF-8 (Windows cp1251 charmap hatasi icin)
import io
...
OUT_DIR = Path(r"C:\tmp\erpide-panel\public\screenshots\pocketerpide")
OUT_DIR.mkdir(parents=True, exist_ok=True)

# Mock auth response — /pocket page check'i (me?.user) icin yeterli
MOCK_ME = {
    "user": {
        "id": "ss-demo-user",
        "email": "demo@erpide.com",
        "name": "Demo",
        "surname": "Kullanici",
    },
    "apps": {"pocketerpide": True},  # hasLicense=true (banner gozukmesin)
}

# Demo data — gercekci ay (Haziran 2026) icin doldurulmus PocketData
DEMO_DATA = {
    "salary": {"gross": 65000, "net": 48700, "payDay": 1},
    "txs": [
        # Haziran 2026 gelirler
    ...
    "goals": [
        {"target": 50000, "deadline": "2026-12-31", "title": "Tatil fonu"},
    ],
    "cards": [
    ...
    "loans": [
        ...
        {"id": "loan-2", "name": "Taşıt Kredisi",  "lender": "Yapikredi",     "principal": 250000, "interestRate": 3.45, "monthlyPayment": 6800,  "startDate": "2025-03-01", "termMonths": 48,  "type": "tasit"},
    ],
    "loanPayments": [
...
# Sekme bilgisi: (tab_key, dosya_adi, baslik_aramak_icin_locator)
TABS = [
    ("genel",    "01-genel-bakis.png",    "Genel Bakış"),
    ("kartlar",  "02-kredi-kartlari.png", "Kredi Kartları"),
    ("krediler", "03-krediler.png",       "Krediler"),
    ("alimlar",  "04-buyuk-alimlar.png",  "Büyük Alımlar"),
]


async def handle_route(route: Route):
    url = route.request.url
    if "/api/shop/auth/me" in url:
        await route.fulfill(
            status=200,
            content_type="application/json",
            body=json.dumps(MOCK_ME),
        )
    else:
...
async def main():
    async with async_playwright() as p:
        ...
        # Inject localStorage via init script (before any page load)
        init_script = f"""
        try {{
            localStorage.setItem('pocket:v2', JSON.stringify({json.dumps(DEMO_DATA)}));
        }} catch (e) {{ console.error('localStorage inject failed:', e); }}
        """
        ...
        # Once Genel'e git, sonra her tab icin button click + SS
        print(f"Navigating to {BASE_URL}/pocket ...")
        await page.goto(f"{BASE_URL}/pocket", wait_until="networkidle", timeout=60000)
        await page.wait_for_timeout(1500)  # framer-motion animasyonlari bitsin

        for tab_key, filename, title_hint in TABS:
            try:
                # Tab button'una tikla (label text ile)
                btn = page.get_by_role("button", name=title_hint)
                if await btn.count() > 0:
                    await btn.first.click()
                    await page.wait_for_timeout(800)
                else:
                    print(f"!!! Tab button bulunamadi: {title_hint}")
