...
/**
 * Gündem akışı. Post verisi server component'ten gelir (DB'den okunur);
 * bu bileşen yalnızca filtreleme ve sunumla ilgilenir.
 */
export default function GundemFeed({ posts }: { posts: SocialPostView[] }) {
  ...
  // browser saatine göre gösterir (banner için yeterli).
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="flex items-center justify-center gap-2 flex-wrap mb-12 sticky top-20 z-10 py-3 -mx-6 px-6 backdrop-blur-lg bg-black/30">
            <FilterChip
              ...
              count={posts.length}
            />
            {(["product-launch", "special-day", "milestone"] as PostKind[]).map((t) => {
              ...
              return (
                <FilterChip
                  ...
                  onClick={() => setFilter(t)}
                  Icon={Icon}
                  label={kindLabel(t, locale)}
                  count={posts.filter((p) => p.kind === t).length}
                />
              );
            })}
          </div>
...
function FeedCard({ post, delay, locale, readMoreLabel }: { post: SocialPostView; delay: number; locale: Locale; readMoreLabel: string }) {
        ...
        <div className="grid md:grid-cols-[280px_1fr] gap-0">
          ...
          <div className="relative aspect-video md:aspect-auto md:min-h-[200px] overflow-hidden">
            {image && (image.includes("/screenshots/") || image.includes("/api/og/")) ? (
              <Image
                ...
                unoptimized={image.includes("/api/og/")}
              />
            ) : image ? (
              <div
          ...
          <div className="p-6 flex flex-col">
            <div className="flex items-center gap-3 mb-3 flex-wrap">
              <span className={`inline-flex items-center gap-1.5 text-[10px] font-bold px-2 py-1 rounded-full border ${KIND_COLORS[post.kind]}`}>
                <Icon size={10} />
                {typeLabel}
              </span>
