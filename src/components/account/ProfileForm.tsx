  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <section className="p-6 rounded-2xl bg-[#111118] border border-white/5">
        <h2 className="font-semibold text-white mb-4">Kişisel Bilgiler</h2>
            ...
            <div className="flex items-center gap-2">
              ...
              {initial.emailVerified ? (
                ...
                <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-amber-500/10 border border-amber-500/20 text-amber-300 text-[11px]">
                  <MailWarning size={11} /> Doğrulanmamış
                </span>
              )}
            </div>
          </div>
          <Field label="GSM (+90...)" value={form.gsmNumber} onChange={(v) => setForm({ ...form, gsmNumber: v })} placeholder="+905..." />
          <Field label="TC Kimlik No" value={form.identityNumber} onChange={(v) => setForm({ ...form, identityNumber: v })} maxLength={11} />
        </div>
      </section>

      <section className="p-6 rounded-2xl bg-[#111118] border border-white/5">
        <h2 className="font-semibold text-white mb-1">Şirket Bilgileri</h2>
        <p className="text-xs text-gray-500 mb-4">Sadece kurumsal fatura kesilecekse doldur (isteğe bağlı).</p>
        <div className="grid sm:grid-cols-2 gap-4">
          <Field label="Şirket Ünvanı" value={form.companyName} onChange={(v) => setForm({ ...form, companyName: v })} />
          <Field label="Vergi No / VKN" value={form.taxNumber} onChange={(v) => setForm({ ...form, taxNumber: v })} maxLength={10} />
        </div>
      </section>
