    ...
    <>
      ...
      {addresses.length === 0 ? (
        <div className="p-12 rounded-2xl bg-[#111118] border border-white/5 text-center">
          ...
          <p className="text-gray-400 mb-1">Henüz kayıtlı adresin yok.</p>
          <p className="text-xs text-gray-500">İlk eklediğin adres otomatik olarak hem fatura hem teslimat adresi olur.</p>
        </div>
      ) : (
    ...
    <div className="p-5 rounded-2xl bg-[#111118] border border-white/5 hover:border-blue-500/20 transition">
      <div className="flex items-start justify-between gap-3 mb-3">
        ...
        <div className="flex gap-1">
          <button onClick={onEdit} className="p-1.5 rounded-lg text-gray-400 hover:bg-white/5 hover:text-white transition" title="Düzenle">
            <Edit2 size={14} />
          </button>
          <button onClick={onDelete} disabled={deleting} className="p-1.5 rounded-lg text-gray-400 hover:bg-red-500/10 hover:text-red-400 disabled:opacity-50 transition" title="Sil">
            <Trash2 size={14} />
          </button>
        </div>
      </div>
      ...
      <div className="mt-4 pt-3 border-t border-white/5 flex flex-wrap gap-2">
        ...
        {addr.isShippingDefault && (
          <span className="inline-flex items-center gap-1 text-[10px] px-2 py-0.5 rounded-full bg-emerald-500/15 text-emerald-300 border border-emerald-500/30">
            <Star size={9} /> Varsayılan Teslimat
          </span>
        )}
      </div>
    </div>
