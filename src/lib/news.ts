...
export const NEWS: NewsPost[] = [
  ...
  {
    id: "dataengine-launch",
    slug: "data-engine-canlida-erp-veri-dogrulama",
    type: "product-launch",
    date: "2026-06-17",
    title: "Data Engine Canlıda — ERP Geçişlerinde Veri Doğrulama Komuta Merkezi",
    excerpt:
      "Eski ERP'den yeniye veri taşırken kayıp, fark, takılı kalan kayıtlar artık tahmin değil. Data Engine canlı dashboard ile her belge tipi için kaynak–hedef sayımı, fark ve drill-down listesi sunar. dataengine.erpide.com üzerinden canlıya çıktı.",
    body:
      "ERPIDE ürün ailesine bugün bir yenisi katıldı: **Data Engine — ERP geçişlerinde veri doğrulama komuta merkezi.**\n\n**Neden Data Engine?**\n\nBir ERP'den başka bir ERP'ye geçiş projelerinin en kritik anı go-live öncesi 48 saattir. Tüm finansal verinin — yıllarca biriken cari bakiyeler, fatura kalemleri, stok hareketleri, demirbaş kartları, ücret bordroları — eski sistemden yeniye eksiksiz aktığını **kanıtlamak** zorundasın. Bu kanıtı manuel Excel kontrollerine bırakırsan: gece 03:00'te bir hata bulup geri dönüş yapamazsın.\n\n**Data Engine bu işi saniyelere indiriyor:**\n\n- **Çift yönlü kaynak/hedef kıyaslaması** — ERP_TO_ACC, ACC_TO_ERP veya BIDIRECTIONAL — hangi yönde aksıyorsa o yönde ölç\n- **Belge tipi seviyesinde fark gösterimi** — \"satınalma faturalarında ERP'de 1.247 var, ACC'ye 1.243 düştü, fark 4 → hangileri?\"\n- **Stuck / Error / Done KPI'ları** — yapışıp kalan kayıtlar otomatik flag, hata mesajıyla birlikte\n- **7 günlük trend grafiği** — sync hızı düşüyor mu, yavaşlama nerede?\n- **Modül bazlı yığılma analizi** — Satınalma'da %98 sync, Üretim'de %71 — odaklan\n- **Mizan + bakiye eşitliği denetimi** — opening + dönem hareketi = closing eşitliği bozulan hesapları flag'le\n- **Drill-down belge listesi** — KPI'a tıkla, hangi belgelerin bekleyip kaldığını gör\n\n**Kurulum mimarisi (güvenlik öncelikli):**\n\nData Engine **müşterinin kendi Windows Server'ına** kurulur. Kaynak ve hedef SQL veritabanlarına LAN üzerinden bağlanır — finansal veri şirket sınırından çıkmaz. Danışman ekibi (ERPIDE veya müşterinin kendi IT'si) Cloudflare Tunnel üzerinden, nereden olursa olsun güvenli HTTPS ile erişir. Token-based tunnel; müşteri admin hesabımıza erişmez.\n\n**Kimler için?**\n\n- 1C, CANIAS, SAP, Logo, Mikro veya başka bir ERP'den geçiş yapan firmalar\n- Çoklu ERP entegrasyonu yöneten danışmanlık firmaları (ACC ↔ ERP gibi çift sistemler)\n- Yıl sonu kapanış öncesi mizan/bakiye doğrulama yapan finans ekipleri\n- Yıllık denetim öncesi sample data validation gereken denetçi ekipler\n\n**Erişim:**\n\nDemo: [dataengine.erpide.com](https://dataengine.erpide.com) — admin demo girişiyle göz at. Kurulum, kaynak/hedef tablo eşleştirmesi, kustom belge tipi tanımları ve canlı destek paketleri için iletişime geç.\n\nERPIDE olarak Data Engine'i ERP geçiş projelerinin standart aracı haline getirme hedefindeyiz. Lansman fiyatı + erken müşteri için özel paket görüşmek istersen, biletin ulaştığı an dönüyoruz.",
    image: "/screenshots/dataengine/01-dashboard.png",
    imageAlt: "Data Engine canlı dashboard — modül bazlı sync sayımı, 7 günlük trend, KPI'lar",
    imageBackground: "#0a0a14",
    gradient: "from-blue-600 to-purple-600",
    badges: ["YENİ ÜRÜN", "BUGÜN", "GLOBAL"],
    productSlug: "dataengine",
    i18n: {
      en: {
        title: "Data Engine Live — Mission Control for ERP Migration Data Validation",
        excerpt: "Loss, mismatches and stuck records during ERP-to-ERP migrations are no longer guesswork. Data Engine's live dashboard shows source–target counts, diffs and a drill-down record list for every document type. Available now at dataengine.erpide.com.",
        body: "A new addition joins the ERPIDE product family today: **Data Engine — mission control for ERP migration data validation.**\n\n**Why Data Engine?**\n\nThe most critical 48 hours of any ERP-to-ERP migration project are the ones before go-live. You have to *prove* that every piece of financial data — years of customer balances, invoice lines, inventory movements, asset cards, payroll records — has fully flowed from the old system to the new one. Leaving that proof to manual Excel checks means at 3 AM you discover an error and there's no turning back.\n\n**Data Engine cuts that work down to seconds:**\n\n- **Two-way source/target comparison** — ERP_TO_ACC, ACC_TO_ERP or BIDIRECTIONAL — measure whichever direction is leaking\n- **Document-type-level diff** — \"there are 1,247 purchase invoices in ERP, 1,243 landed in ACC, diff is 4 → which ones?\"\n- **Stuck / Error / Done KPIs** — records frozen mid-pipeline are flagged automatically with the error message\n- **7-day trend chart** — is sync slowing down, and where?\n- **Module-level backlog** — Purchasing at 98% synced, Production at 71% — focus where it hurts\n- **Trial balance + balance equality audit** — flag accounts where opening + period activity ≠ closing\n- **Drill-down document list** — click a KPI to see exactly which records are stuck\n\n**Deployment architecture (security-first):**\n\nData Engine is installed on **the customer's own Windows Server**. It connects to source/target SQL databases over the LAN — financial data never crosses the corporate perimeter. The consulting team (ERPIDE or the customer's IT) reaches it from anywhere over Cloudflare Tunnel with secure HTTPS. Token-based tunnel; the customer does not gain access to our admin account.\n\n**Who is it for?**\n\n- Companies migrating from 1C, CANIAS, SAP, Logo, Mikro or any other ERP\n- Consultancies running multi-ERP integrations (dual systems like ACC ↔ ERP)\n- Finance teams validating trial balance and account balances before year-end close\n- Auditors needing sample data validation before annual audit\n\n**Get started:**\n\nDemo: [dataengine.erpide.com](https://dataengine.erpide.com) — log in with the admin demo and explore. For deployment, source/target table mapping, custom document types and ongoing support, please get in touch.\n\nAt ERPIDE our goal is to make Data Engine the default tool for ERP migration projects. Reach out for launch pricing and early-adopter packages — we respond as soon as your message lands.",
      },
      ru: {
        title: "Data Engine в эфире — командный центр проверки данных при миграции ERP",
        excerpt: "Потери, расхождения и застрявшие записи при переходе с одной ERP на другую больше не догадки. Живой дашборд Data Engine показывает счётчики источника-приёмника, разницу и drill-down список записей по каждому типу документов. Доступен на dataengine.erpide.com.",
        body: "К семье продуктов ERPIDE сегодня присоединяется новый участник: **Data Engine — командный центр проверки данных при миграции ERP.**\n\n**Зачем Data Engine?**\n\nСамые критичные 48 часов любого проекта перехода с одной ERP на другую — это часы перед go-live. Вам нужно *доказать*, что все финансовые данные — годами накопленные сальдо клиентов, строки счетов-фактур, движения запасов, карточки основных средств, расчётные ведомости — полностью перешли из старой системы в новую. Если оставите эту проверку на ручной Excel, то в 3 часа ночи найдёте ошибку и обратной дороги уже не будет.\n\n**Data Engine сокращает эту работу до секунд:**\n\n- **Двусторонняя сверка источник/приёмник** — ERP_TO_ACC, ACC_TO_ERP или BIDIRECTIONAL — измеряйте то направление, где течёт\n- **Разница на уровне типа документа** — \"в ERP 1 247 закупочных счетов, в ACC попало 1 243, разница 4 → какие именно?\"\n- **KPI Stuck / Error / Done** — записи, застрявшие в середине, автоматически помечаются с сообщением ошибки\n- **График тренда за 7 дней** — синхронизация замедляется, и где?\n- **Бэклог по модулям** — Закупки 98% синхронизировано, Производство 71% — фокусируйтесь там, где больно\n- **Аудит оборотно-сальдовой ведомости** — счета, где opening + движение ≠ closing, помечаются\n- **Drill-down по документам** — кликните по KPI, чтобы увидеть конкретные застрявшие записи\n\n**Архитектура развёртывания (приоритет — безопасность):**\n\nData Engine устанавливается на **собственный Windows Server заказчика**. Подключается к SQL источника/приёмника по локальной сети — финансовые данные не покидают периметр компании. Команда консультантов (ERPIDE или ИТ заказчика) подключается откуда угодно через Cloudflare Tunnel по защищённому HTTPS. Токен-туннель; заказчик не получает доступ к нашему административному аккаунту.\n\n**Кому подходит?**\n\n- Компаниям, мигрирующим с 1C, CANIAS, SAP, Logo, Mikro или любой другой ERP\n- Консалтинговым фирмам, ведущим мульти-ERP интеграции (двойные системы вроде ACC ↔ ERP)\n- Финансовым командам, проверяющим оборотно-сальдовую ведомость и сальдо перед закрытием года\n- Аудиторам, которым нужна выборочная проверка данных перед годовым аудитом\n\n**Доступ:**\n\nДемо: [dataengine.erpide.com](https://dataengine.erpide.com) — войдите с демо-админом и изучите. Для развёртывания, сопоставления таблиц источника/приёмника, кастомных типов документов и пакетов поддержки — свяжитесь с нами.\n\nERPIDE ставит цель сделать Data Engine стандартным инструментом проектов миграции ERP. Запросите цены лансмана и пакеты для ранних клиентов — отвечаем сразу.",
      },
      kk: {
        title: "Data Engine іске қосылды — ERP көшу жобаларында деректер валидациясының командалық орталығы",
        excerpt: "ERP-тен ERP-ке көшу кезінде деректер жоғалуы, сәйкессіздіктер мен жабысып қалған жазбалар енді болжам емес. Data Engine тікелей дашборды әрбір құжат түрі үшін көз-мақсат санағын, айырмашылықты және drill-down тізімін көрсетеді. dataengine.erpide.com сайтында қол жетімді.",
        body: "Бүгін ERPIDE өнімдер отбасына жаңа мүше қосылды: **Data Engine — ERP көшу жобаларында деректер валидациясының командалық орталығы.**\n\n**Неге Data Engine?**\n\nКез келген ERP-тен ERP-ке көшу жобасының ең маңызды 48 сағаты — go-live алдындағы сағаттар. Сіз барлық қаржы деректерінің — жылдар бойы жинақталған клиент сальдолары, фактура жолдары, қор қозғалыстары, ОҚ карталары, жалақы ведомостары — ескі жүйеден жаңасына толық өткенін *дәлелдеуге* міндеттісіз. Бұл дәлелдеуді қолмен Excel тексеруіне қалдырсаңыз, түнгі 3-те қате тауып, артқа қайту мүмкіндігі болмайды.\n\n**Data Engine осы жұмысты секундтарға қысқартады:**\n\n- **Екі жақты көз/мақсат салыстыруы** — ERP_TO_ACC, ACC_TO_ERP немесе BIDIRECTIONAL — қай бағытта ағатынын өлшеңіз\n- **Құжат түрі деңгейінде айырмашылық** — \"ERP-те 1 247 сатып алу фактурасы бар, ACC-қа 1 243 түсті, айырмашылық 4 → қайсылары?\"\n- **Stuck / Error / Done KPI-лары** — ортада жабысып қалған жазбалар қате хабарламасымен бірге автоматты белгіленеді\n- **7 күндік тренд графигі** — синхрондау баяулауда ма, қайда?\n- **Модуль бойынша бэклог** — Сатып алу 98% синхрондалған, Өндіріс 71% — қай жерде ауыр, сол жерге назар салыңыз\n- **Айналым сальдо мен бакиа теңдігі аудиті** — opening + кезең қозғалысы ≠ closing шотты белгіленеді\n- **Құжаттар бойынша drill-down** — KPI-ға басып, нақты қандай жазбалар тұрып қалғанын көріңіз\n\n**Ендіру архитектурасы (қауіпсіздік басымдықпен):**\n\nData Engine **тапсырыс берушінің өз Windows Server-іне** орнатылады. Дереккөзі мен мақсатты SQL деректер базаларына LAN арқылы қосылады — қаржы деректері компания периметрінен шықпайды. Кеңесшілер тобы (ERPIDE немесе тапсырыс берушінің IT қызметі) Cloudflare Tunnel арқылы кез келген жерден қауіпсіз HTTPS-пен қол жеткізеді. Токен-туннель; тапсырыс беруші біздің админ есептік жазбамызға қол жеткізе алмайды.\n\n**Кімдерге арналған?**\n\n- 1C, CANIAS, SAP, Logo, Mikro немесе басқа ERP-тен көшетін компанияларға\n- Бірнеше ERP интеграциясын басқаратын кеңес беру компанияларына (ACC ↔ ERP сияқты қос жүйелер)\n- Жыл соңы жабылуы алдында айналым сальдосы мен шот сальдосын тексеретін қаржы командаларына\n- Жылдық аудит алдында іріктеп тексеру қажет аудиторларға\n\n**Қол жеткізу:**\n\nДемо: [dataengine.erpide.com](https://dataengine.erpide.com) — демо-админмен кіріп қарап шығыңыз. Ендіру, көз/мақсат кесте сәйкестендіруі, арнайы құжат түрлері мен қолдау пакеттері үшін бізбен хабарласыңыз.\n\nERPIDE мақсаты — Data Engine-ді ERP көшу жобаларының стандартты құралына айналдыру. Лансман бағасы мен ерте клиенттер пакеті туралы сұрау салсаңыз, хабарыңыз жеткен сәтте жауап береміз.",
      },
    },
  },
];
